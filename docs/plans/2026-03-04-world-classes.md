...
### Task 6: Character creation form — class selector

The lobby character creation form lives in PR 07 (not yet built). This task adds the class selector component that will be used there. We build it now so it's ready to drop in.

**Files:**
- Create: `components/campaign/ClassSelector.tsx`

**Step 1: Create the component**

```typescript
// components/campaign/ClassSelector.tsx
"use client";

import { WorldClass } from "@/types/world";
...
export function ClassSelector({ classes, value, onChange }: ClassSelectorProps) {
  return (
    <div className="space-y-2">
      <label className="text-xs uppercase tracking-widest text-ash">
        Character Class
      </label>
        ...
        {classes.map((cls) => (
          <button
            ...
            className={[
              ...
              "bg-iron border-gunmetal hover:border-brass",
              value === cls.name
                ? "border-brass shadow-[0_0_12px_rgba(196,148,61,0.3)] text-bone"
                : "text-ash",
            ].join(" ")}
          >
            <div className="font-semibold text-sm tracking-wide">{cls.name}</div>
            <div className="text-xs mt-0.5 text-ash">{cls.description}</div>
          </button>
        ))}
      </div>
    </div>
...
```bash
git add components/campaign/ClassSelector.tsx
git commit -m "feat: add ClassSelector component for character creation"
```
